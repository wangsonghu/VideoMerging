    ...
    for item in file_list:
        with open(item, encoding='utf-8', mode='r') as f:
            ...
            with open(ffmpeg_cmd_list, encoding='utf-8', mode='a+') as fp:
                print('sudo docker run -it --rm --name ffmpeg -v .:/config -w /config harbor.hosts.songhu.wang:8443/apps/linuxserver/ffmpeg:amd64-version-6.0-cli_v2023-08-09 -i %s -i %s -vcodec copy -acodec copy "[%s.][%s][%s·%s][%s].mkv"' % (audio_file, video_file, ep_index, ep_bvid, title, ep_index_title, quality_pithy_description), file=fp)
